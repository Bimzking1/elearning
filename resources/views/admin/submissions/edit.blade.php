...
<div class="w-full mx-auto bg-white p-6 rounded-xl shadow-md space-y-8">
    ...
    <div class="bg-gray-50 border rounded-lg p-6 space-y-4">
        ...
        <div class="border-t pt-4">
            <h4 class="text-sm font-semibold text-gray-800 mb-2">Attachment File</h4>
    ...
    <div class="bg-white border border-gray-200 rounded-xl p-6 shadow-sm space-y-6">
        ...
        <div>
            <strong class="text-sm font-medium text-gray-700">Submitted At:</strong>
            <span class="text-sm text-gray-600">{{ $submission->created_at->format('d M Y, H:i') }}</span>
        </div>
        ...
        <div>
            <strong class="text-sm font-medium text-gray-700">Time Status:</strong>
            <span class="text-sm font-semibold {{ $statusClass }}">{{ $difference }} ({{ $status }})</span>
        </div>
    </div>

    {{-- Score Form --}}
    <form action="{{ route('admin.tasks.submissions.update', ['task' => $submission->task->id, 'submission' => $submission->id]) }}"
          method="POST"
