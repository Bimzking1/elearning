...
<div class="max-w-6xl mx-auto bg-white p-6 rounded-lg shadow-md">
    ...
    <form action="{{ route('admin.tasks.update', $task->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        @csrf
        @method('PUT')
            ...
            @if(auth()->user()->role == 'admin')
                <select name="teacher_id" id="teacher_id" required
                    ...
                    @foreach($teachers as $teacher)
                        <option value="{{ $teacher->id }}" {{ old('teacher_id', $task->teacher_id) == $teacher->id ? 'selected' : '' }}>
                            @php
                                $specializations = is_array($teacher->specialization)
                                    ? $teacher->specialization
                                    : json_decode($teacher->specialization, true) ?? [];
                            @endphp
                            {{ $teacher->user->name }}@if($specializations) - ({{ implode(', ', $specializations) }})@endif
                        </option>
                    @endforeach
                </select>
            @else
                @if(auth()->user()->teacher)
                    <input type="text" value="{{ auth()->user()->teacher->name }}" readonly
                ...
                @else
                    <p class="text-red-600 text-sm">You do not have a teacher assigned.</p>
                @endif
            @endif
            @error('teacher_id')
                <p class="text-red-600 text-sm">{{ $message }}</p>
