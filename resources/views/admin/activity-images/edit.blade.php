...
<div class="max-w-7xl mx-auto bg-white p-6 rounded-lg shadow-md">
    ...
    <h2 class="text-2xl font-bold mb-6 text-gray-800">Edit Activity Image</h2>
    ...
    <form action="{{ route('admin.activity-images.update', $image) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <img src="{{ asset('storage/' . $image->image_path) }}"
                 alt="{{ $image->title }}"
                 class="h-48 rounded-lg border border-gray-200 shadow-sm object-contain">
        </div>
        ...
        <div class="flex items-start gap-3 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
            <input type="checkbox" name="is_pinned" id="is_pinned" value="1"
                {{ old('is_pinned', $image->is_pinned) ? 'checked' : '' }}
                class="mt-0.5 w-4 h-4 accent-yellow-500 cursor-pointer">
            <div>
                <label for="is_pinned" class="text-sm font-medium text-gray-800 cursor-pointer">
                    📌 Pin this image
                </label>
                <p class="text-xs text-gray-500 mt-0.5">
                    Pinned images appear at the top of the activities page and the latest 4 pinned appear on the homepage.
                </p>
            </div>
        </div>
