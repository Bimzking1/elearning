    ...
    <form action="{{ route('student.tasks.store', $task->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-5">
            ...
            <textarea name="submission_text" id="submission_text" rows="5"
                      class="w-full border border-gray-300 rounded-md shadow-sm px-3 py-2 focus:ring focus:ring-blue-200 focus:outline-none">{{ old('submission_text') }}</textarea>
        ...
        <div class="flex justify-end">
            <button type="submit"
                    class="bg-blue-500 hover:bg-blue-600 text-white font-semibold px-5 py-2 rounded-md shadow transition text-sm">
                Submit Task
            </button>
        </div>
    </form>
