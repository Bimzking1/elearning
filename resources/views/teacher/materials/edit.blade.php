    ...
    <form action="{{ route('teacher.materials.update', $material->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <select name="subject_id" required class="w-full border border-gray-300 rounded-lg p-2 mt-1">
                @foreach($subjects as $subj)
                    <option value="{{ $subj->id }}" {{ $material->subject_id == $subj->id ? 'selected' : '' }}>
                        {{ $subj->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
                ...
                @forelse($material->link_urls ?? [] as $url)
                    <div class="flex items-center gap-2">
                        ...
                        <button type="button" onclick="removeLinkField(this)"
                                class="text-red-500 hover:text-red-700 mt-1 text-sm">Remove</button>
                    </div>
                @empty
                    <div class="flex items-center gap-2">
            ...
            <button type="button" onclick="addLinkField()"
                    class="text-sm text-blue-600 hover:underline mt-2">+ Add another link</button>
        </div>
